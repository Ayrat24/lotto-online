(function () {
  ...
  // --- in-page debug console (visible overlay) -------------------------------------------------
  // Provides a way to capture logs when Developer Tools are not available
  // (e.g. Telegram Desktop embedded webview). A small DBG toggle appears in
  // the corner; click to open an overlay that shows recent debug lines.
  (function createMiniappDebugUI() {
    try {
      var dbgToggle = document.createElement('button');
      dbgToggle.id = 'miniappDebugToggle';
      dbgToggle.textContent = 'DBG';
      dbgToggle.style.position = 'fixed';
      dbgToggle.style.right = '10px';
      dbgToggle.style.bottom = '10px';
      dbgToggle.style.zIndex = 99999;
      dbgToggle.style.background = 'rgba(0,0,0,0.6)';
      dbgToggle.style.color = '#fff';
      dbgToggle.style.border = '1px solid rgba(255,255,255,0.08)';
      dbgToggle.style.borderRadius = '6px';
      dbgToggle.style.padding = '6px 8px';
      dbgToggle.style.fontSize = '12px';
      dbgToggle.style.cursor = 'pointer';
      dbgToggle.style.fontFamily = 'Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif';
      dbgToggle.title = 'Toggle miniapp debug console (for support)';

      var dbgConsole = document.createElement('div');
      dbgConsole.id = 'miniappDebugConsole';
      dbgConsole.style.position = 'fixed';
      dbgConsole.style.right = '10px';
      dbgConsole.style.bottom = '48px';
      dbgConsole.style.width = '380px';
      dbgConsole.style.maxHeight = '40vh';
      dbgConsole.style.overflow = 'auto';
      dbgConsole.style.background = 'rgba(6,10,16,0.9)';
      dbgConsole.style.color = '#dfefff';
      dbgConsole.style.fontSize = '12px';
      dbgConsole.style.lineHeight = '1.2';
      dbgConsole.style.padding = '8px';
      dbgConsole.style.borderRadius = '8px';
      dbgConsole.style.border = '1px solid rgba(255,255,255,0.06)';
      dbgConsole.style.boxShadow = '0 8px 24px rgba(0,0,0,0.6)';
      dbgConsole.style.zIndex = 99998;
      dbgConsole.style.display = 'none';
      dbgConsole.style.whiteSpace = 'pre-wrap';

      var dbgClear = document.createElement('button');
      dbgClear.textContent = 'Clear';
      dbgClear.style.display = 'block';
      dbgClear.style.marginBottom = '6px';
      dbgClear.style.fontSize = '11px';
      dbgClear.style.padding = '4px 6px';
      dbgClear.style.cursor = 'pointer';
      dbgClear.addEventListener('click', function () { dbgConsole.innerHTML = ''; });

      dbgConsole.appendChild(dbgClear);

      function __miniapp_append_debug_ui() {
        try {
          if (document && document.body) {
            document.body.appendChild(dbgToggle);
            document.body.appendChild(dbgConsole);
          }
        } catch (e) {}
      }

      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', __miniapp_append_debug_ui);
      } else {
        __miniapp_append_debug_ui();
      }

      dbgToggle.addEventListener('click', function () {
        try {
          dbgConsole.style.display = dbgConsole.style.display === 'none' ? 'block' : 'none';
        } catch (e) {}
      });

      function appendMiniappDebugLine(level, args) {
        try {
          var el = document.getElementById('miniappDebugConsole');
          if (!el) return;
      ...
      // Mirror console.log/error to in-page console so user can copy output.
      try {
        var __origLog = console.log.bind(console);
        var __origError = console.error.bind(console);
        console.log = function () { __origLog.apply(console, arguments); appendMiniappDebugLine('LOG', arguments); };
        console.error = function () { __origError.apply(console, arguments); appendMiniappDebugLine('ERR', arguments); };
      } catch (e) {}
    } catch (e) {}
  })();
